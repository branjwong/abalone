import { getNeighbors, getOptions, Position } from './Utility';
...
describe('getOptions', () => {
    ...
    ])('should throw an error when a selectedPosition is not a doubled coordinate', ({ selection }) => {
        expect(getOptions(selection)).toThrow('Not doubled coordinate: ' + JSON.stringify(selection));
    });
    ...
    ])('should throw an error when a selectedPosition is out of bounds', ({ selection }) => {
        expect(getOptions(selection)).toThrow('Out of bounds: ' + JSON.stringify(selection));
    });
